...
/* Capacitor reactive power ratings (VAR) */
const int C_VAR[6] = {100, 200, 400, 800, 1600, 3200};
...
void loop() {
  ...
  /* -------- POWER CALCULATIONS -------- */
  float S = Vrms * Irms;                 // Apparent Power
  float P = S * pf;                      // Active Power
  float Q = S * sin(acos(pf));           // Load Reactive Power
  ...
  digitalWrite(C1, cap[0]);
  digitalWrite(C2, cap[1]);
  digitalWrite(C3, cap[2]);
  digitalWrite(C4, cap[3]);
  digitalWrite(C5, cap[4]);
  digitalWrite(C6, cap[5]);
  ...
  // Line 2: PF & S (Apparent Power)
  lcd.setCursor(0, 1);
  lcd.print("                    ");
  lcd.setCursor(0, 1);
  lcd.print("PF:");
  dtostrf(pf, 4, 3, buf); lcd.print(buf);
  lcd.print(" S:");
  dtostrf(S, 5, 0, buf); lcd.print(buf);
  ...
  lcd.setCursor(0, 2);
  lcd.print("P:");
  dtostrf(P, 5, 0, buf); lcd.print(buf);
  lcd.print(" Q:");
  dtostrf(Q, 5, 0, buf); lcd.print(buf);
